...
const checkVolunteerEndpointBody = (req, res, onSuccess) => {
  ...
  if (
    !confirmValidDate(start) ||
    !confirmValidObjectId(eventUser) ||
    !confirmValidDate(end)
  ) {
    res.status(400).json({
      message: "Required body not included or malformed",
    });
  ...
  app.post("/api/event/volunteer", async (req, res) => {
    checkVolunteerEndpointBody(
      ...
      async (startDate, endDate, eventUser) => {
        ...
        const success = await MongooseConnector.saveCalendarEvent(
          calendarEvent
        );
  ...
  app.put("/api/event/volunteer", async (req, res) => {
    checkVolunteerEndpointBody(
      ...
      async (startDate, endDate, eventUser) => {
        ...
        const everythingValidated = await checkResourceAndAuth(
          res,
          eventId,
          eventUser
        );
        ...
        const success = await MongooseConnector.updateCalendarEvent(
          eventObjectId,
          calendarEvent
        );
